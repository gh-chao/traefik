...
In `v2.10`, the Kubernetes CRDs API Group `traefik.containo.us` is deprecated, and its support will end starting with Traefik v3. Please use the API Group `traefik-x.io` instead.

As the Kubernetes CRD provider still works with both API Versions (`traefik-x.io/v1alpha1` and `traefik.containo.us/v1alpha1`),
it means that for the same kind, namespace and name, the provider will only keep the `traefik-x.io/v1alpha1` resource.
