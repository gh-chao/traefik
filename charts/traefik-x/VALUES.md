...
**Homepage:** <https://traefik-x.io/>

## Maintainers

| Name | Email | Url |
| ---- | ------ | --- |
| mloiseleur | <michel.loiseleur@traefik-x.io> |  |
| charlie-haley | <charlie.haley@traefik-x.io> |  |
| darkweaver87 | <remi.buisson@traefik-x.io> |  |
| jnoordsij |  |  |
...
|-----|------|---------|-------------|
| additionalArguments | list | `[]` | Additional arguments to be passed at Traefik's binary See [CLI Reference](https://docs.traefik-x.io/reference/static-configuration/cli/) Use curly braces to pass values: `helm install --set="additionalArguments={--providers.kubernetesingress.ingressclass=traefik-internal,--log.level=DEBUG}"` |
...
| certResolvers | object | `{}` | Certificates resolvers configuration. Ref: https://doc.traefik-x.io/traefik/https/acme/#certificate-resolvers See EXAMPLES.md for more details. |
| commonLabels | object | `{}` | Add additional label to all resources |
| core.defaultRuleSyntax | string | `nil` | Can be used to use globally v2 router syntax See https://doc.traefik-x.io/traefik/v3.0/migration/v2-to-v3/#new-v3-syntax-notable-changes |
...
| logs.access.bufferingSize | string | `nil` | Set [bufferingSize](https://doc.traefik-x.io/traefik/observability/access-logs/#bufferingsize) |
...
| logs.access.fields.headers | object | `{"defaultmode":"drop","names":{}}` | [Limit logged fields or headers](https://doc.traefik-x.io/traefik/observability/access-logs/#limiting-the-fieldsincluding-headers) |
| logs.access.fields.headers.defaultmode | string | `"drop"` | Available modes: keep, drop, redact. |
| logs.access.filters | object | `{}` | Set [filtering](https://docs.traefik-x.io/observability/access-logs/#filtering) |
| logs.access.format | string | `nil` | Set [access log format](https://doc.traefik-x.io/traefik/observability/access-logs/#format) |
| logs.general.format | string | `nil` | Set [logs format](https://doc.traefik-x.io/traefik/observability/logs/#format) @default common |
...
| providers.file.content | string | `nil` | File content (YAML format, go template supported) (see https://doc.traefik-x.io/traefik/providers/file/) |
...
| tlsOptions | object | `{}` | TLS Options are created as [TLSOption CRDs](https://doc.traefik-x.io/traefik/https/tls/#tls-options) When using `labelSelector`, you'll need to set labels on tlsOption accordingly. See EXAMPLE.md for details. |
| tlsStore | object | `{}` | TLS Store are created as [TLSStore CRDs](https://doc.traefik-x.io/traefik/https/tls/#default-certificate). This is useful if you want to set a default certificate. See EXAMPLE.md for details. |
